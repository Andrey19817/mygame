...
export const MonstrWindow = ({ name, avatar, xp, strong, armor, check, id }) => {
    ...
    return (
        <div className={`div_container_monstr ${!opacity ? 'opacity' : null}`} >
            <img style={{ width: '100px', height: '100px' }} src={avatar} alt=""  onClick={() => {//блокировка на удаление первого монстра
            ...
            <div className="container_characteristic">
                <div className="characteristic">
                    Жизнь - <input className="input_monstr_window"  value={valueLife} onChange={(event)=>{setValueLife(event.target.value)}}></input>
                </div>
                <div className="characteristic">
                    Сила - <input className="input_monstr_window" type="text"  value={valueStrong} onChange={(event)=>{setValueStrong(event.target.value)}} />
                </div>
                <div className="characteristic">
                    Броня - <input className="input_monstr_window" type="text"  value={valueArmor} onChange={(event)=>{setValueArmor(event.target.value)}} />
                </div>
                <button className="button_reestablish" onClick={()=>{dispatch({type:ContainerActivType.life_editing,payload:{
                    id:id,
                    xp:valueLife,
                    strong:valueStrong,
                    armor:valueArmor
