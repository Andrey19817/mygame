...
export function ContainerModal({
  ...
  return (
    <div className="wrapper_modal">
      <div>.</div>
      {pers.bestPers.level && showBestPers ? (
        <div className="best_pers">
          Лучший игрок - {pers.bestPers.name} Уровень - {pers.bestPers.level}
        </div>
      ) : null}
      {show_input ? (
          ...
          <button
            ...
            onClick={() => {
              if (value !== "" && value !== "admin") {
                ...
                fetch(
                  "https://api.giphy.com/v1/gifs/search?api_key=c48Uu72vB5HOjYdeCrJcg82EkOmH9MDg&q=dark+souls&limit=30&offset=0&rating=g&lang=en"
                )
      ...
      {show_image_selection ? (
        <div className="container_show_image_selection">
          <p>Выбери Аватарку</p>
          <div
            className="show_image_selection"
            onClick={(event) => {
              setShow_image_selection(false);
          ...
          >
            {getPicture.map((elem) => (
              <Avatar key={nanoid(10)} avatar={elem.images.original.url} />
            ))}
          </div>
        </div>
      ) : null}
      {!show_image_selection && !show_input && !admin ? (
        <HeroWindow
          value={value}
          avatar={showAvatar}
          return_image_selection={return_image_selection}
        />
      ) : null}
          ...
          <button
            ...
            onClick={() => {
              if (value !== "") {
                ...
                isAudioPlay && audioClick.play();
                dispatch({
                  type: ContainerActivType.getAvatar,
                  payload: showAvatar,
                });
                play();
              }
